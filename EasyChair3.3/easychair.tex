...
%
\author{
Ilshat Aliev\inst{1}\thanks{Designed the word-document}
\and
    Abhilash Srikantha\inst{2}\thanks{Designed the latex document}
\and
Christoph Lange\inst{1,2}\thanks{Reviewed the documents}\\
}

% Institutes for affiliations are also joined by \and,
\institute{
  Informatik IV, Universit\"at Bonn,
  Germany\\
  \email{aliev@dummy.de, lange@dummy.com}
\and
   Informatik III, Universit\"at Bonn,
   Germany\\
   \email{srikantha@dummy.edu}\\
 }
...
\authorrunning{Aliev, Srikantha and Lange}
...
\begin{abstract}
	...
	to the Computer Science Conference for University of Bonn Students. It
	also greatly borrows from the format of the British Machine Vision Conference. 
	
	\textbf{All authors are encouraged to read this document}, even if you have
	...
	errors that are common even in the work of authors who \textit{have}
	written many papers before.

	Further, in order to ease the lives of authors, editors, and trees, we present an
...
\label{sec:intro}
The proceedings of CSCUBS are published in electronic form, but it is still assumed
that readers of the papers may wish to print the paper. This document
illustrates the required paper format.
...
example.  Note that best results will be achieved using pdflatex,
which is available in most modern distributions.

Readers are referred to~\cite{easyChairDocuments} about detailed information
regarding the easychair class of this template document. 

\subsection{Paper length plus bibliography}
Full papers must be 10-12~pages in length and short papers must be 4--6~pages 
\textit{including} the bibliography. \textbf{Papers which are
...
full paper for presentation in twelve pages if it is reviewed in fourteen.

The bibliography should begin immediately after the paper text.

\begin{figure}[tb]
	\begin{centering}
	\includegraphics[width=0.25\textwidth]{unibonn_logo.jpg}
	\caption{It is often a good idea for the first figure to attempt to
	encapsulate the article, complementing the abstract. }
	\label{fig:unibonn_logo}
	\end{centering}
\end{figure}
...
\begin{enumerate}
\item
The default paper size is US letter, explicitly set to A4 
(\texttt{a4paper}) paper in the document class entry, e.g.:\\\verb+\documentclass[a4paper]{easychair}+

\item
The print area is 145x224 mm. This size has been selected to 
allow for inexpensive printing using our current print-on-demand 
publisher.
...
Figure~\ref{fig:unibonn_logo}. If the papers are designed for
...
\author{
    Serguei A. Mokhov\inst{1}\thanks{Designed and implemented class style}
\and
    Geoff Sutcliffe\inst{2}\thanks{Did numerous tests}
\and
   Andrei Voronkov\inst{3}\thanks{Masterminded EasyChair }\\
\and
...
Larger formulae must be done using {\verb|\|[}~~{\verb|\|]}
bracketing, e.g.,
\[
\int_{0}^{1} x dx = \left[ \frac{1}{2}x^2 \right]_{0}^{1} = \frac{1}{2}
\]
or using {\verb|\|begin\{equation\}} and {\verb|\|end\{equation\}} for
numbered equations, e.g.,
\begin{equation}
e^x = \sum_{n=0}^\infty \frac{x^n}{n!} = \lim_{n\rightarrow\infty} (1+x/n)^n
\end{equation}

Use {\verb|\|begin\{align*\}} and {\verb|\|end\{align*\}} (or without
...
class to \url{studconf2014-oc@lists.iai.uni-bonn.de}.
...
dimensions. Figure~\ref{fig:cscubs-logo} on page~\pageref{fig:cscubs-logo}
...
\begin{figure}[tb]
	\begin{centering}
	\includegraphics[width=0.75\textwidth]{cscubs_logo.png}\\ \vspace{20pt}
	\includegraphics[width=0.50\textwidth]{cscubs_logo.png}\\ \vspace{20pt}
	\includegraphics[width=0.25\textwidth]{cscubs_logo.png}\\
	\caption{sample image }
	\label{fig:cscubs-logo}
	\end{centering}
\end{figure}
...
Figure~\ref{fig:cscubs-logo} shows three copies of the same image
having different sizes obtained using the following commands:

\begin{verbatim}
  \includegraphics[width=0.75\textwidth]{cscubs_logo.png}
  \includegraphics[width=0.50\textwidth]{cscubs_logo.png}
  \includegraphics[width=0.25\textwidth]{cscubs_logo.png}
\end{verbatim}
